Note: Find some mp3 that back to 20 years ago, find there are quite a few garbled texts. try to fix with exsiting tools but failed. so create this local tools to fix the issue.
...
Python Library Fallback: Uses mutagen/eyeD3 when FFmpeg fails
